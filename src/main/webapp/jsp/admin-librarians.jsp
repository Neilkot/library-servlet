	...
	<div class="container h-100">
		<div class="row h-100 align-items-center justify-content-center text-center ">
			...
			<div class="container-sm align-self-baseline">
				...
				<form id="submit_form" action="/admin-librarians" method="post">
					<input id="submit_librarian_id" name="id" type="hidden" /> <input id="submit_request_type" name="method" value="delete" type="hidden" />
				</form>
				<a href="/jsp/admin-add-librarian.jsp"><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#create-modal">
						...
						<tbody>
							<c:forEach var="librarian" items="${librarians}">
								<tr>
									...
									<td><button type="button" class="btn btn-warning" onclick="submitRequest(${librarian.id});">
			...
			<nav aria-label="Page navigation example">
				<ul class="pagination justify-content-right">
					<c:if test="${offset != 0}">
						<li class="page-item"><a class="page-link" href="/admin-librarians?offset=${pageSize * currPage - pageSize * 2}"><fmt:message key="pagination.previous" /></a></li>
					</c:if>
					<c:forEach begin="1" end="${noOfPages}" var="i">
						<c:choose>
							...
							<c:otherwise>
								<li class="page-item"><a class="page-link" href="/admin-librarians?offset=${i * pageSize - pageSize}">${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${((pageSize * noOfPages) - pageSize) gt offset}">
						<li class="page-item"><a class="page-link" href="/admin-librarians?offset=${pageSize * currPage}"><fmt:message key="pagination.next" /></a></li>
					</c:if>
				</ul>
			</nav>
		</div>
